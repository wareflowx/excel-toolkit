...
from excel_toolkit.fp import Result, err, is_err, ok, unwrap_err
...
def validate_sort_columns(
    ...
    if missing:
        return err(ColumnNotFoundError(column=missing[0], available=list(df.columns)))
...
def sort_dataframe(
    ...
    try:
        ...
        error_msg = str(e)
        if (
            "not comparable" in error_msg
            or "unorderable types" in error_msg
            or "mixed types" in error_msg
            or "not supported between instances" in error_msg
        ):
            # Find the problematic column
            for col in columns:
                if df[col].dtype == "object":
                    return err(NotComparableError(column=col, message=error_msg))
            return err(NotComparableError(column=columns[0], message=error_msg))
